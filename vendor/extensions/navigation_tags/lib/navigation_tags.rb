module NavigationTags
  ...
  desc %{
    ...
    * @relative@ defaults to false, takes as root current page
  ...
  tag "nav" do |tag|
    relative = tag.attr['relative'] == 'true' ? true : false
    
    if relative
      root = tag.locals.page
    else
  ...
  def not_allowed? child_page
    ...
    !child_page.show_in_menu? or child_page.virtual? or !child_page.published? or child_page.part("no-map")  
  end
