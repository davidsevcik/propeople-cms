class SearchPage < Page
  ...
  def render
    ...
    when 'postgresql'
      sql_content_check = "((lower(page_parts.content) ILIKE ?) OR (lower(pages.title) ILIKE ?))"
    else
      sql_content_check = "((LOWER(page_parts.content) LIKE ?) OR (LOWER(pages.title) LIKE ?))"
    end
    unless (@query = q.to_s.strip).blank?
      tokens = query.split.collect { |c| "%#{c.mb_chars.downcase}%"}      
      source = Page.respond_to?(:current_site) ? Page.current_site.pages : Page
      pages = source.find(:all, :order => 'published_at DESC', :include => [ :parts ],
...
If you need to exclude some pages from the search results you can specify their URLs in exclude_pages attribute separated by comma. Dont forget trailing slash 
